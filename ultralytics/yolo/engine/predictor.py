...
# Flask app para el servicio web
app = Flask(__name__)
latest_frame = None  # Variable global para almacenar la última imagen
...
class BasePredictor:
    ...
    def show(self, p):
        ...
        #cv2.imshow(str(p), im0) #comentado pfierro
        #cv2.waitKey(1)  # 1 millisecond

        #Para enviarla al servidor de medios
        # (4) Colocar frame procesado en el buffer para el servidor de video
        """
        Modifica la función 'show' para almacenar la imagen y exponerla a través de un servidor web.
        """
        global latest_frame
        ...
        # Convertir la imagen a formato JPEG utilizando PIL
        #image = Image.fromarray(im0_rgb)
        #buffer = BytesIO()
        ...
        # (Opcional) Mostrar la imagen localmente utilizando OpenCV
        #cv2.imshow("Prediction", im0)
        #cv2.waitKey(1)

        """
        Modifica la función 'show' para actualizar la imagen global
        y permitir transmitirla como un video feed.
        """
        global latest_frame

        # Actualizar el último frame con im0
        latest_frame = im0

        # Mostrar la imagen localmente (opcional)
...
# Generador para transmitir frames como un stream de video
def video_stream():
    global latest_frame
    while True:
        if latest_frame is not None:
            ...
            # Enviar el frame a los clientes (stream)
            yield (b'--frame\r\n'
                   b'Content-Type: image/jpeg\r\n\r\n' + frame + b'\r\n')
        else:
            # Si no hay frame disponible, espera un momento
            yield (b'--frame\r\n'
                   b'Content-Type: text/plain\r\n\r\n' +
                   b'No frame available yet.\r\n')


# Ruta Flask para transmitir el stream de video
@app.route('/video_feed')
def video_feed():
    """
    Endpoint para transmitir la secuencia de video en tiempo real.
    """
    return Response(video_stream(), mimetype='multipart/x-mixed-replace; boundary=frame')



# Ruta Flask para servir la imagen
@app.route("/image")
def serve_image():
    """
    Endpoint para obtener la última imagen generada.
    """
    global latest_frame
...
# Inicia el servidor Flask al inicio de la aplicación
